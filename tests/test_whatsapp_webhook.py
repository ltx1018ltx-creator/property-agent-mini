...
class WebhookTests(unittest.TestCase):
    ...
    def test_verification_access_log_omits_query_values(self):
        access_log=io.StringIO()
        with redirect_stderr(access_log):
            status,_,_=self.request('GET','/api/whatsapp/webhook?hub.mode=subscribe&hub.verify_token=verify-me&hub.challenge=secret-challenge')
        self.assertEqual(status,200)
        output=access_log.getvalue()
        self.assertIn('GET /api/whatsapp/webhook HTTP/1.1',output)
        self.assertNotIn('verify-me',output)
        self.assertNotIn('secret-challenge',output)
        self.assertNotIn('hub.verify_token',output)
