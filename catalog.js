...
function enhanceFilterSelect(el){
  ...
  if(searchable){
    ...
    searchInput.oninput=()=>{const query=searchInput.value.trim().toLowerCase();[...choices.querySelectorAll('[data-option-index]')].forEach(button=>button.hidden=Boolean(query)&&!button.textContent.toLowerCase().includes(query));el._locationQuery=query;el._syncFilterUI();render()};
  }
  choices.addEventListener('click',event=>{const button=event.target.closest('button');if(!button||!choices.contains(button))return;if(button.dataset.clearFilter){CatalogFilters.clearSelect(el);el._clearCustomFilter?.()}else CatalogFilters.toggleOption(el,Number(button.dataset.optionIndex));el._syncFilterUI();el.dispatchEvent(new Event('change',{bubbles:true}))});
  el.dataset.enhanced='true';label.replaceWith(group);group.append(summary,...(searchInput?[searchInput]:[]),choices,el);
  el._clearCustomFilter=()=>{if(!searchInput)return;searchInput.value='';el._locationQuery='';[...choices.querySelectorAll('[data-option-index]')].forEach(button=>button.hidden=false)};
  el._syncFilterUI=()=>{const selected=CatalogFilters.selectedValues(el),buttons=[...choices.querySelectorAll('[data-option-index]')],typed=Boolean(el._locationQuery);buttons.forEach(button=>{const on=el.options[Number(button.dataset.optionIndex)].selected;button.classList.toggle('active',on);button.setAttribute('aria-pressed',String(on))});anyButton.classList.toggle('active',!selected.length&&!typed);anyButton.setAttribute('aria-pressed',String(!selected.length&&!typed));summary.querySelector('b').textContent=selected.length?`${selected.length} selected`:typed?'Searching':'Any';group.classList.toggle('has-selection',Boolean(selected.length)||typed)};
  el._syncFilterUI();
}
