...
function render(){const locationEl=$('#locationFilter'),filters={search:$('#search').value.trim().toLowerCase(),deal:CatalogFilters.selectedValues($('#dealFilter')),location:CatalogFilters.selectedValues(locationEl),locationQuery:locationEl._locationQuery||'',type:CatalogFilters.selectedValues($('#typeFilter')),subtype:CatalogFilters.selectedValues($('#subtypeFilter')),tenure:CatalogFilters.selectedValues($('#tenureFilter')),price:CatalogFilters.selectedValues($('#priceFilter'))},rows=CatalogFilters.filterListings(listings,filters,{type:propertyType,subtype,searchType:type}),active=filters.deal.length+filters.location.length+filters.type.length+filters.subtype.length+filters.tenure.length+filters.price.length+(filters.locationQuery?1:0);$('#filterBadge').textContent=active;$('#filterBadge').classList.toggle('hidden',!active);$('#count').textContent=`${rows.length} propert${rows.length===1?'y':'ies'}`;$('#list').innerHTML=rows.length?rows.map(card).join(''):'<div class="empty">No matching properties found.<br>Try clearing some filters.</div>'}
...
(async()=>{const q=new URLSearchParams(location.search),agent=q.get('agent'),id=q.get('id');try{if(agent)listings=await loadAgentListings(agent);else if(id){const rows=await sbJson(`/rest/v1/public_shares?id=eq.${encodeURIComponent(id)}&select=payload`),payload=rows[0]?.payload;if(!payload?.collection)throw Error();listings=payload.listings||[]}else throw Error();const fill=(el,values)=>[...new Set(values.filter(usable))].sort().forEach(value=>el.add(new Option(value,value)));fill($('#locationFilter'),[...listingLocations,...listings.map(x=>x.location)]);fill($('#typeFilter'),propertyTypes);fill($('#subtypeFilter'),propertySubtypes);Array.from({length:20},(_,i)=>({label:`${(i+1)*100}k`,value:`${i?i*100000+1:0}-${(i+1)*100000}`})).concat({label:'Above 2m',value:'2000001-Infinity'}).forEach(({label,value})=>$('#priceFilter').add(new Option(label,value)));['dealFilter','locationFilter','typeFilter','subtypeFilter','tenureFilter','priceFilter'].forEach(id=>enhanceFilterSelect($('#'+id)));render()}catch(e){console.error(e);$('#count').textContent='';$('#list').innerHTML='<div class="empty">This property catalog is unavailable.</div>'}})();
