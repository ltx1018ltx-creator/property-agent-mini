...
(async()=>{const q=new URLSearchParams(location.search),agent=q.get('agent'),id=q.get('id');try{if(agent)listings=await loadAgentListings(agent);else if(id){let rows;try{rows=await sbJson(`/rest/v1/public_shares?id=eq.${encodeURIComponent(id)}&select=payload`)}catch{throw Error('catalog_request_failed')}const payload=rows[0]?.payload;if(!payload?.collection)throw Error('catalog_response_invalid');listings=(payload.listings||[]).map(value=>CatalogData.normalizeListing(value,{id:value?.id})).filter(Boolean)}else throw Error('catalog_response_invalid')}catch(error){const category=error?.message==='catalog_request_failed'?'catalog_request_failed':'catalog_response_invalid';console.error(category);$('#count').textContent='';$('#list').innerHTML=category==='catalog_request_failed'?'<div class="empty">This property catalog is unavailable.</div>':'<div class="empty">No properties are available right now.</div>';return}try{const fill=(el,values)=>[...new Set(values.filter(usable))].sort().forEach(value=>el.add(new Option(value,value)));fill($('#locationFilter'),[...listingLocations,...listings.map(x=>x.location)]);fill($('#typeFilter'),propertyTypes);fill($('#subtypeFilter'),propertySubtypes);Array.from({length:20},(_,i)=>({label:`${(i+1)*100}k`,value:`${i?i*100000+1:0}-${(i+1)*100000}`})).concat({label:'Above 2m',value:'2000001-Infinity'}).forEach(({label,value})=>$('#priceFilter').add(new Option(label,value)));['dealFilter','locationFilter','typeFilter','subtypeFilter','tenureFilter','priceFilter'].forEach(id=>enhanceFilterSelect($('#'+id)));render()}catch{console.error('catalog_render_failed');$('#count').textContent='0 properties';$('#list').innerHTML='<div class="empty">No matching properties found.</div>'}})();
