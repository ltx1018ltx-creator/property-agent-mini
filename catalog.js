...
function enhanceFilterSelect(el){
  ...
  if(searchable){
    ...
    searchInput.oninput=()=>{const query=searchInput.value.trim().toLowerCase();[...choices.children].forEach(button=>button.hidden=Boolean(query)&&!button.textContent.toLowerCase().includes(query));el._locationQuery=query;el._syncFilterUI();render()};
  }
  el.dataset.enhanced='true';label.replaceWith(group);group.append(summary,...(searchInput?[searchInput]:[]),choices,el);
  el._clearCustomFilter=()=>{if(!searchInput)return;searchInput.value='';el._locationQuery='';[...choices.children].forEach(button=>button.hidden=false)};
  el._syncFilterUI=()=>{const selected=[...el.selectedOptions],buttons=[...choices.children],typed=Boolean(el._locationQuery);buttons.forEach((button,index)=>{const on=el.options[index].selected;button.classList.toggle('active',on);button.setAttribute('aria-pressed',String(on))});summary.querySelector('b').textContent=selected.length?`${selected.length} selected`:typed?'Searching':'Any';group.classList.toggle('has-selection',Boolean(selected.length)||typed)};
  el._syncFilterUI();
}
...
function render(){const q=$('#search').value.trim().toLowerCase(),deal=selectedValues($('#dealFilter')),locationEl=$('#locationFilter'),loc=selectedValues(locationEl),locationQuery=locationEl._locationQuery||'',pt=selectedValues($('#typeFilter')),st=selectedValues($('#subtypeFilter')),tenure=selectedValues($('#tenureFilter')),ranges=selectedValues($('#priceFilter')),rows=listings.filter(x=>(!q||[x.location,type(x),x.tenure,x.price,x.deal].some(v=>String(v||'').toLowerCase().includes(q)))&&(!deal.length||deal.some(v=>String(x.deal||'').toLowerCase().includes(v)))&&includesOrAll(loc,x.location)&&(!locationQuery||String(x.location||'').toLowerCase().includes(locationQuery))&&includesOrAll(pt,propertyType(x))&&includesOrAll(st,subtype(x))&&includesOrAll(tenure,x.tenure)&&matchesPrice(ranges,x.price)),active=deal.length+loc.length+pt.length+st.length+tenure.length+ranges.length+(locationQuery?1:0);$('#filterBadge').textContent=active;$('#filterBadge').classList.toggle('hidden',!active);$('#count').textContent=`${rows.length} propert${rows.length===1?'y':'ies'}`;$('#list').innerHTML=rows.length?rows.map(card).join(''):'<div class="empty">No matching properties found.<br>Try clearing some filters.</div>'}
...
$('#clearFilters').onclick=()=>{['dealFilter','locationFilter','typeFilter','subtypeFilter','tenureFilter','priceFilter'].forEach(id=>{const el=$('#'+id);[...el.options].forEach(o=>o.selected=false);el._clearCustomFilter?.();el._syncFilterUI?.()});$('#search').value='';render()};
