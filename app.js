...
$('#deleteSelectedListings').onclick=()=>{const ids=new Set([...selectedListings].map(Number)),count=db.listings.filter(x=>ids.has(Number(x.id))).length;if(!count)return;if(!confirm(`Delete ${count} selected listing${count===1?'':'s'}? This cannot be undone.`))return;db.listings=db.listings.filter(x=>!ids.has(Number(x.id)));selectedListings.clear();updateBulkBar();save();toast(`${count} listing${count===1?'':'s'} deleted`)};
