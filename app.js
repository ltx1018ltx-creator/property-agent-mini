...
function listingHTML(x){const type=x.propertyType||x.title||'Property',details=[x.location,x.tenure,x.lotType,x.furnishing,x.renovation].filter(v=>v&&v!=='Not Applicable'&&v!=='Not Specified').join(' · '),photos=x.photos?.length?x.photos:(x.photo?[x.photo]:[]),checked=selectedListings.has(x.id);return `<article class="item listing-item clickable" onclick="viewListing(${Number(x.id)})"><label class="listing-check" onclick="event.stopPropagation()"><input type="checkbox" ${checked?'checked':''} onchange="toggleListing(${Number(x.id)},this.checked)"></label>${photos[0]?`<div class="listing-photo-wrap"><img class="listing-thumb" src="${photos[0]}" alt="${esc(type)}">${photos.length>1?`<span>+${photos.length-1}</span>`:''}</div>`:'<span class="avatar">⌂</span>'}<div class="info"><b>${esc(type)}</b><small>${esc(details)}</small>${x.landSize||x.builtUp?`<small>${x.landSize?'Land '+esc(x.landSize)+' sqft':''}${x.landSize&&x.builtUp?' · ':''}${x.builtUp?'Built-up '+esc(x.builtUp)+' sqft':''}</small>`:''}</div><div class="listing-tail"><b>${money(x.price)}</b><div class="listing-actions"><button onclick="event.stopPropagation();editListing(${Number(x.id)})">Edit</button><button onclick="event.stopPropagation();shareListing(${Number(x.id)})">Share</button></div></div>${itemControls('listings',x.id)}</article>`}
...
function normalizeImportedListing(x){
 ...
 merged.rawText=raw;merged.title=merged.title||[merged.storeys,merged.propertyType].filter(v=>v&&v!=='Not Applicable').join(' ');
 return merged;
}
