...
const propertyTypes=['Terrace House','Semi-D','Bungalow','Townhouse','Cluster House','Condominium','Apartment','Flat','Serviced Residence','SOHO / Studio','Shoplot','Retail Lot','Office','Warehouse / Factory','Detached Factory','Semi-D Factory','Terrace Factory','Agricultural Land','Residential Land','Commercial Land','Industrial Land'];
...
function canonicalPropertyType(v){return ({'Terrace / Link House':'Terrace House','Semi-Detached House':'Semi-D','Bungalow / Detached House':'Bungalow','Detached House':'Bungalow'})[v]||v}
function propertySubtypeOf(x){return normalizedSubtype(x?.propertySubtype||x?.storeys||'')}
function listingHTML(x){const type=canonicalPropertyType(x.propertyType)||x.title||'Property',location=x.location||'Location not specified',details=[type,propertySubtypeOf(x),x.tenure,x.lotType].filter(v=>v&&v!=='Not Applicable'&&v!=='Not Specified').join(' · '),photos=x.photos?.length?x.photos:(x.photo?[x.photo]:[]),checked=selectedListings.has(x.id),size=[x.landSize&&`Land ${x.landSize} sqft`,x.builtUp&&`Built-up ${x.builtUp} sqft`].filter(Boolean).join(' · ');return `<article class="listing-card clickable" onclick="viewListing(${Number(x.id)})"><label class="listing-check" onclick="event.stopPropagation()"><input type="checkbox" ${checked?'checked':''} onchange="toggleListing(${Number(x.id)},this.checked)"></label>${photos[0]?`<div class="listing-photo-wrap"><img class="listing-thumb" src="${photos[0]}" alt="${esc(type)}">${photos.length>1?`<span>+${photos.length-1}</span>`:''}</div>`:'<span class="listing-placeholder">⌂</span>'}<div class="listing-card-body"><div class="listing-card-top"><span class="listing-deal">${esc(x.deal||'Listing')}</span><b class="listing-price">${money(x.price)}</b></div><h3>${esc(location)}</h3><p>${esc(details)}</p>${size?`<small>${esc(size)}</small>`:''}<div class="listing-card-actions"><button onclick="event.stopPropagation();editListing(${Number(x.id)})">Edit</button><button onclick="event.stopPropagation();shareListing(${Number(x.id)})">Share</button><button class="delete-listing" onclick="event.stopPropagation();deleteItem('listings',${Number(x.id)})">Delete</button></div></div></article>`}
...
function filterListings(){const q=$('#listingSearch').value.trim().toLowerCase(),loc=$('#listingLocationFilter').value,type=$('#listingTypeFilter').value,subtype=$('#listingSubtypeFilter').value,tenure=$('#listingTenureFilter').value,range=$('#listingPriceFilter').value;let [min,max]=range?range.split('-').map(Number):[0,Infinity];const a=db.listings.filter(x=>{const pt=canonicalPropertyType(x.propertyType),pst=propertySubtypeOf(x);return(!q||[x.location,pt,pst,x.deal,x.price,x.tenure].some(v=>String(v||'').toLowerCase().includes(q)))&&(!loc||x.location===loc)&&(!type||pt===type)&&(!subtype||pst===subtype)&&(!tenure||x.tenure===tenure)&&Number(x.price)>=min&&Number(x.price)<=max});const active=[loc,type,subtype,tenure,range].filter(Boolean).length;$('#listingFilterBadge').textContent=active;$('#listingFilterBadge').classList.toggle('hidden',!active);$('#listingResultCount').textContent=`${a.length} listing${a.length===1?'':'s'}`;$('#listingList').innerHTML=a.length?a.map(listingHTML).join(''):'<div class="empty">没有符合条件的房源。试试清除 filters。</div>'}
...
window.deleteItem=(kind,id)=>{const labels={leads:'lead',listings:'listing',cases:'case'};if(!db[kind]||!confirm(`Delete this ${labels[kind]||'item'}?`))return;const target=String(id);db[kind]=db[kind].filter(x=>String(x.id)!==target);if(kind==='listings')[...selectedListings].forEach(v=>{if(String(v)===target)selectedListings.delete(v)});updateBulkBar();save();toast('Deleted')};
...
$('#listingSearch').oninput=filterListings;
...
$('#clearListingFilters').onclick=()=>{['listingLocationFilter','listingTypeFilter','listingSubtypeFilter','listingTenureFilter','listingPriceFilter'].forEach(id=>$('#'+id).value='');$('#listingSearch').value='';filterListings()};
...
function parseWhatsAppListing(text){
 ...
 const typeRules=[['Detached Factory',/detached\s+(?:factory|warehouse)|独立(?:式)?工厂/],['Semi-D Factory',/semi[- ]?d\s+(?:factory|warehouse)/],['Terrace Factory',/terrace(?:d)?\s+(?:factory|warehouse)/],['Warehouse / Factory',/warehouse|factory|kilang|工厂|货仓|厂房/],['Terrace House',/terrace|link house|rumah\s+teres|排屋/],['Semi-D',/semi[- ]?d(?:etached)?(?!\s*(?:factory|warehouse))|rumah\s+berkembar|半独立/],['Bungalow',/bungalow|detached house|rumah\s+sesebuah|独立式住宅/],['Townhouse',/town\s*house|townhouse/],['Cluster House',/cluster house/],['Serviced Residence',/serviced (?:residence|apartment)/],['Condominium',/condo|minium/],['Apartment',/apartment|pangsapuri|公寓/],['Flat',/\bflat\b|组屋/],['SOHO / Studio',/\bsoho\b|studio unit/],['Shoplot',/shop ?lot|shop ?house|kedai|店屋/],['Retail Lot',/retail (?:lot|space)/],['Office',/office|pejabat|办公/],['Industrial Land',/industrial land|tanah industri|工业地/],['Residential Land',/residential land|tanah kediaman|住宅地/],['Commercial Land',/commercial land|tanah komersial|商业地/],['Agricultural Land',/agricultural land|agri land|tanah pertanian|农地/]];
...
function normalizeImportedListing(x){
 ...
 merged.propertyType=canonicalPropertyType(merged.propertyType);merged.propertySubtype=propertySubtypeOf(merged);merged.rawText=raw;merged.title=merged.title||[merged.propertySubtype,merged.propertyType].filter(v=>v&&v!=='Not Applicable').join(' ');
 return merged;
}
...
$('#saveListing').onclick=async e=>{e.preventDefault();const f=$('#listingForm');if(!f.reportValidity())return;const x=Object.fromEntries(new FormData(f));delete x.photoFile;x.title=[x.propertySubtype,x.propertyType].filter(v=>v&&v!=='Not Applicable').join(' ');const files=[...$('#listingPhoto').files];if(files.length>10)return toast('最多只能上传 10 张照片');if(files.length)x.photos=await Promise.all(files.map(compressPhoto));let listing,isEdit=Boolean(x.id);if(isEdit){const i=db.listings.findIndex(v=>String(v.id)===x.id),old=db.listings[i];listing={...old,...x,id:Number(x.id),shareId:''};delete listing.storeys;if(!files.length)listing.photos=old.photos||[];db.listings[i]=listing}else{listing={...x,id:Date.now()};db.listings.unshift(listing)}try{save()}catch(err){return toast('照片总容量太大，请减少照片或换较小的照片')}$('#listingDialog').close();toast(isEdit?'Listing updated':`Listing saved${files.length?' · '+files.length+' photos':''}`);try{await publishListing(listing);save()}catch(err){toast('Listing 已保存，share link 暂时生成不到')}};
...
window.viewListing=async id=>{const x=db.listings.find(v=>v.id===id);if(!x)return;const tab=open('about:blank','_blank');try{const url=x.shareId?`${location.origin}/share.html?id=${x.shareId}`:await publishListing(x);save();if(tab)tab.location=url;else location.href=url}catch(e){if(tab)tab.close();toast('Listing 页面暂时开不到')}};
...
$('#shareSelectedListings').onclick=async()=>{const listings=db.listings.filter(x=>selectedListings.has(x.id));if(!listings.length)return;try{const id=await publishShare({collection:true,listings}),url=`${location.origin}/share.html?id=${id}`;if(navigator.share)await navigator.share({title:`${listings.length} Property Listings`,url});else{await navigator.clipboard.writeText(url);toast('Listing collection link copied')}}catch(e){if(e?.name!=='AbortError')toast('合集网址生成失败，请再试')}};
$('#deleteSelectedListings').onclick=()=>{const ids=new Set([...selectedListings].map(String)),count=db.listings.filter(x=>ids.has(String(x.id))).length;if(!count)return;if(!confirm(`Delete ${count} selected listing${count===1?'':'s'}? This cannot be undone.`))return;db.listings=db.listings.filter(x=>!ids.has(String(x.id)));selectedListings.clear();updateBulkBar();save();toast(`${count} listing${count===1?'':'s'} deleted`)};
