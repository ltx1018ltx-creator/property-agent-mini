...
$('#clearListingFilters').onclick=()=>{['listingLocationFilter','listingTypeFilter','listingSubtypeFilter','listingTenureFilter','listingPriceFilter'].forEach(id=>$('#'+id).value='');$('#listingSearch').value='';resetListingPage()};
...
window.viewListing=id=>{const x=db.listings.find(v=>String(v.id)===String(id));if(!x)return;const photos=x.photos?.length?x.photos:(x.photo?[x.photo]:[]),type=canonicalPropertyType(x.propertyType)||'Property',subtype=propertySubtypeOf(x),encoded=encodeURIComponent(String(x.id)),facts=[
 ['Property',[subtype,type].filter(Boolean).join(' · ')],
 ['Tenure',x.tenure],
 ['Land size',x.landSize&&`${x.landSize} sqft`],
 ['Built-up',x.builtUp&&`${x.builtUp} sqft`],
 ['Bedrooms',x.bedrooms],
 ['Bathrooms',x.bathrooms],
 ['Car parks',x.carParks],
 ['Lot type',x.lotType]
 ].filter(([,v])=>v&&v!=='N/A'&&v!=='Not Specified'&&v!=='Not Applicable');
 $('#listingViewContent').innerHTML=`<button type="button" class="dialog-close listing-view-close" onclick="closeListingView()" aria-label="Close">×</button>${photos.length?`<div class="listing-view-gallery">${photos.map((p,i)=>`<img src="${p}" alt="${esc(type)} photo ${i+1}" loading="${i?'lazy':'eager'}" decoding="async">`).join('')}</div>`:'<div class="listing-view-no-photo">⌂</div>'}<div class="listing-view-body"><span class="listing-deal">${esc(x.deal||'Listing')}</span><h2>${esc(x.location||'Location not specified')}</h2><strong>${money(x.price)}</strong><div class="listing-view-facts">${facts.map(([k,v])=>`<div><small>${esc(k)}</small><b>${esc(v)}</b></div>`).join('')}</div><div class="listing-view-actions"><button onclick="closeListingView();editListing(decodeURIComponent('${encoded}'))">Edit</button><button class="primary" onclick="shareListing(decodeURIComponent('${encoded}'))">Share</button></div></div>`;
