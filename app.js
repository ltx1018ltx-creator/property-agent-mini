...
$('#customerCatalogBtn').onclick=async()=>{if(!session?.user?.id)return toast('Please log in again');const listings=db.listings.filter(x=>String(x._ownerId||session.user.id)===String(session.user.id));try{const id=await publishShare({collection:true,listings,catalog:true}),url=`${location.origin}/catalog.html?id=${encodeURIComponent(id)}`;try{await navigator.clipboard.writeText(url);toast('Customer listing link copied')}catch{prompt('Copy customer listing link:',url)}}catch(e){toast(e.message||'Customer link 暂时生成不到')}};
