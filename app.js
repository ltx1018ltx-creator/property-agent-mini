...
window.viewListing=async id=>{const x=db.listings.find(v=>String(v.id)===String(id));if(!x)return;const tab=open('about:blank','_blank');try{const url=x.shareId?`${location.origin}/share.html?id=${x.shareId}`:await publishListing(x);save();if(tab)tab.location=url;else location.href=url}catch(e){if(tab)tab.close();toast('Listing 页面暂时开不到')}};
