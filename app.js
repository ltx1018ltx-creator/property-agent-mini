...
$('#customerCatalogBtn').onclick=async()=>{if(!session?.user?.id)return toast('Please log in again');const url=`${location.origin}/catalog.html?agent=${encodeURIComponent(session.user.id)}`;try{await navigator.clipboard.writeText(url);toast('Customer listing link copied')}catch{prompt('Copy customer listing link:',url)}};
