...
function fillListingOptions(){const add=(el,items,first='')=>{el.innerHTML=(first?`<option value="">${first}</option>`:'')+items.map(x=>`<option>${esc(x)}</option>`).join('')},locations=[...new Set([...listingLocations,...(db?.listings||[]).map(x=>x.location).filter(Boolean)])].sort();$('#listingLocationOptions').innerHTML=locations.map(x=>`<option value="${esc(x)}">`).join('');add($('#listingPropertyType'),propertyTypes,'Choose property type');add($('#listingLocationFilter'),locations);add($('#listingTypeFilter'),propertyTypes);add($('#listingSubtypeFilter'),propertySubtypes);add($('#leadPropertyType'),propertyTypes,'Any property type')}
