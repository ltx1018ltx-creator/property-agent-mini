...
function normalizeImportedListing(x){
 const raw=x.rawText||x.raw_text||x.description||x.text||'',parsed=raw?parseWhatsAppListing(raw):{},aliases={
  ...
  price:['sellingPrice','askingPrice','rental'],storeys:['storey','stories','floors','floorCount','propertySubtype'],
 ...
 merged.propertyType=canonicalPropertyType(merged.propertyType);merged.rawText=raw;merged.title=merged.title||[merged.storeys,merged.propertyType].filter(v=>v&&v!=='Not Applicable').join(' ');
 return merged;
}
