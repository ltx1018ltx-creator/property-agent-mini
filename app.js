...
function enhanceListingFilter(el){
 if(!el||el.dataset.enhanced)return;const label=el.closest('label'),name=label?.childNodes[0]?.textContent.trim();if(!label||!name)return;
 ...
 [...el.options].forEach((option,index)=>{const button=document.createElement('button');button.type='button';button.textContent=option.textContent;button.setAttribute('aria-pressed','false');button.onclick=()=>{option.selected=!option.selected;el._syncFilterUI();el.dispatchEvent(new Event('change',{bubbles:true}))};button.dataset.optionIndex=index;choices.append(button)});
 el.dataset.enhanced='true';label.replaceWith(group);group.append(summary,choices,el);el._syncFilterUI=()=>{const selected=[...el.selectedOptions];[...choices.children].forEach((button,index)=>{const on=el.options[index].selected;button.classList.toggle('active',on);button.setAttribute('aria-pressed',String(on))});summary.querySelector('b').textContent=selected.length?`${selected.length} selected`:'Any';group.classList.toggle('has-selection',Boolean(selected.length))};el._syncFilterUI()
}
...
function renderDraftInbox(aiEnabled){$('#draftInbox').innerHTML=draftSubmissions.length?draftSubmissions.map(s=>{const d=s.draft||{},facts=d.structured_data||{};const texts=s.messages.filter(m=>m.message_type==='text'&&m.message_text);const images=s.messages.filter(m=>m.image_url);return `<article class="card draft-card" data-submission="${esc(s.id)}"><header><div><b>Received ${esc(adminDate(s.started_at))}</b><small>${texts.length} text message${texts.length===1?'':'s'} · ${images.length} image${images.length===1?'':'s'}</small></div><span class="draft-status">${esc(d.status||'not generated')}</span></header>${texts.map(m=>`<div class="draft-message">${esc(m.message_text)}</div>`).join('')}${images.length?`<div class="draft-images">${images.map(m=>`<img src="${esc(m.image_url)}" alt="Private submission image">`).join('')}</div>`:''}<div class="draft-actions"><button class="generate" ${aiEnabled?'':'disabled'}>${d.id?'Regenerate AI Draft':'Generate AI Draft'}</button></div>${d.id?`<div class="draft-fields">${DRAFT_FIELDS.map(k=>`<label>${esc(k)}<input data-field="${esc(k)}" value="${esc(facts[k]??'')}"></label>`).join('')}</div><textarea class="draft-copy" aria-label="Chinese marketing copy">${esc(d.marketing_copy||'')}</textarea><div class="draft-actions"><button class="approve">Approve</button><button class="reject">Reject</button></div>`:''}</article>`}).join(''):'<div class="empty">No WhatsApp submissions yet.</div>';$$('.draft-card .generate').forEach(b=>b.onclick=()=>generateDraft(b));$$('.draft-card .approve').forEach(b=>b.onclick=()=>reviewDraft(b,'approved'));$$('.draft-card .reject').forEach(b=>b.onclick=()=>reviewDraft(b,'rejected'))}
async function loadDraftInbox(){if(!isAdmin)return;try{const data=await adminApi('/api/admin/listing-submissions');draftSubmissions=data.submissions||[];renderDraftInbox(data.ai_enabled)}catch(e){$('#draftInbox').innerHTML=`<div class="empty">${esc(e.message)}</div>`}}
